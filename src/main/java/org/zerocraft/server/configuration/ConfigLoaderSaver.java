		...
		try {
			...
			for (Field field : fields) {
				...
				try {
					...
					ZeroCraftServer.logger.warn("Unable to load configuration field \"" + 
							field.getName() + "\": " + ex.getMessage());
				}
			}
		} catch (Exception ex) {
			ZeroCraftServer.logger.severe("Unable to load configuration: " +
					ex.getMessage());
		}
	}
	...
	public void save() {
		try {
			try {
				...
				ZeroCraftServer.logger.severe("Unable to create configuration file \"" + 
			...
			for (Field field : fields) {
				...
				try {
					...
					ZeroCraftServer.logger.warn("Unable to save configuration field \"" + 
							field.getName() + "\": " + ex.getMessage());
				}
			}
			...
			ZeroCraftServer.logger.severe("Unable to save configuration: " +
					ex.getMessage());
		}
	}
