...
public class Utils {
	...
	public static boolean isNumeric(String str, boolean checkForDouble) {
		if (str == null || !checkForDouble && str.contains(".") || checkForDouble && !str.contains(".")) {
			return false;
		}
	...
	public static String replaceLast(String str, String match, String replaceWith) {
		return new StringBuilder(new StringBuilder(str).reverse().toString().replaceFirst(match, replaceWith)).reverse()
				.toString();
	}
	 ...
	 * From <a href=
	 * "https://stackoverflow.com/a/366532">https://stackoverflow.com/a/366532</a>
	 *
	 ...
	 * From <a href=
	 * "https://stackoverflow.com/a/3760193">https://stackoverflow.com/a/3760193</a>
	 *
	 * @param str       the string to split
	...
	public static String[] splitStringIntoChunks(String str, int chunkSize) {
		List<String> chunks = new ArrayList<>((str.length() + chunkSize - 1) / chunkSize);
	 ...
	 * From <a href=
	 * "https://stackoverflow.com/a/929107">https://stackoverflow.com/a/929107</a>
	 *
	 ...
	 * @return the re-mapped value
	 */
	public static float remapToRange(float oldValue, float oldMin, float oldMax, float min, float max) {
		float oldRange = oldMax - oldMin;
		float range = max - min;
		float value = (oldValue - oldMin) * range / oldRange + min;
		return value;
	}
	...
	/**
	 * Writes the specified RSA public or private key into the specified
	 * FileOutputStream in PEM format
	 *
	 * @param key    the key to write
	 * @param stream the stream to write to
	 * @throws IOException              if an I/O error has occurred
	 * @throws IllegalArgumentException if the key is not a public or private RSA
	 *                                  key
	 */
	...
	public static RSAPublicKey readRSAPublicKeyPEM(File file)
		...
		byte[] publicKey = decoder.decode(fileData.replace("-----BEGIN PUBLIC KEY-----", "").replace("\n", "")
				.replace("\r", "").replace("-----END PUBLIC KEY-----", "").trim());
	...
	public static RSAPrivateKey readRSAPrivateKeyPEM(File file)
		...
		String fileData = new String(Files.readAllBytes(file.toPath()), StandardCharsets.US_ASCII);
		Base64.Decoder decoder = Base64.getDecoder();
		byte[] privateKey = decoder.decode(fileData.replace("-----BEGIN PRIVATE KEY-----", "").replace("\n", "")
				.replace("\r", "").replace("-----END PRIVATE KEY-----", "").trim());
	...
	public static void killOtherJavaInstances() {
		try {
			...
			Process taskList = Runtime.getRuntime().exec("tasklist /FI \"imagename eq javaw.exe\" /FO CSV /NH");
			taskList.waitFor();
	...
	/**
	 * Right pads the specified String with the specified character to the specified
	 * length
	 *
	...
	/**
	 * Clamps the specified value between the specified bounds
	 *
	...
	/**
	 * Registers into the TLS ssl context a trustmanager that trusts all
	 * certificates
	 *
	 * @throws KeyManagementException
	 * @throws NoSuchAlgorithmException
	 */
	public static void registerTrustAllSSL() throws KeyManagementException, NoSuchAlgorithmException {
	...
	/**
	 * Splits the specified byte array into chunks of the specified size.<br>
	 * The last chunk is padded with zeros.
	 * 
	 * @param bytes the bytes
	 * @param chunkSize the size of each chunk
	 * @return the chunks
	 */
	public static List<byte[]> splitBytesIntoChunks(byte[] bytes, int chunkSize) {
